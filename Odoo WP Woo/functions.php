...
function odoo_product_options_general_product_data() {
    ...
    echo '<p>' . esc_html__( 'Please select the fields that you don not want to syncronize.', 'woocommerce' ) . '</p>';
    ...
    woocommerce_wp_checkbox( array(
        'id'          => 'odoo_sync_name',
        'value'       => empty( $odoo_meta ) ? NULL : $odoo_meta['sync_name'],
        'label'       => esc_html__( 'Name', 'woocommerce' ),
        'description' => esc_html__( 'Do not synchronize the product name from odoo.', 'woocommerce' )
    ) );
    echo '</div>';
    ...
    woocommerce_wp_checkbox( array(
        'id'          => 'odoo_sync_price',
        'value'       => empty( $odoo_meta ) ? NULL : $odoo_meta['sync_price'],
        'label'       => esc_html__( 'Price', 'woocommerce' ),
        'description' => esc_html__( 'Do not synchronize the product price from odoo.', 'woocommerce' )
    ) );
    /*woocommerce_wp_text_input( array(
        'id'          => 'odoo_sync_price_note',
        'value'       => empty( $odoo_meta ) ? NULL : $odoo_meta['sync_price_note'],
        'label'       => esc_html__( 'Notification', 'woocommerce' ),
        'desc_tip'    => true,
        'description' => esc_html__( 'The notification shown to the user, if this synchronisation is turned off.', 'woocommerce' )
    ) );*/
    echo '</div>';
    ...
    woocommerce_wp_checkbox( array(
        'id'          => 'odoo_sync_stock',
        'value'       => empty( $odoo_meta ) ? NULL : $odoo_meta['sync_stock'],
        'label'       => esc_html__( 'Stock', 'woocommerce' ),
        'description' => esc_html__( 'Do not synchronize the product stock from odoo.', 'woocommerce' )
    ) );
    echo '</div>';
...
function odoo_save_as_metadata( $id, $post ) {
    update_post_meta( $id, '_odoo', array(
        'sync_name'       => isset( $_POST['odoo_sync_name'] ) ? $_POST['odoo_sync_name'] : false,
        'sync_price'      => isset( $_POST['odoo_sync_price'] ) ? $_POST['odoo_sync_price'] : false,
        //'sync_price_note' => isset( $_POST['odoo_sync_price_note'] ) ? $_POST['odoo_sync_price_note'] : false,
        'sync_stock'      => isset( $_POST['odoo_sync_stock'] ) ? $_POST['odoo_sync_stock'] : false
    ) );
...
/**
 * ---------------------------
 * Add price notification
 * @since Version 1.0
 * ---------------------------
 */

/*function odoo_add_price_notification() {
    $odoo_meta = get_post_meta( get_the_ID(), '_odoo', true );
    if( !empty( $odoo_meta ) && !empty( $odoo_meta['sync_price'] ) && !empty( $odoo_meta['sync_price_note'] ) ) {
        echo ' <small>'.htmlspecialchars( $odoo_meta['sync_price_note'] ).'</small>';
    }
} add_action( 'woocommerce_single_product_after_net_price', 'odoo_add_price_notification' );*/
...
function odoo_get_cron_event_settings() {
    ...
    $settings = array(
        ...
        'odoo_company_id' => array(
            'title'    => esc_html__( 'Company', 'woocommerce' ),
            'desc'     => esc_html__( 'The company you wish to synchronize to.', 'woocommerce' ),
            'id'       => 'odoo_company_id',
            'type'     => 'text',
            'class'    => 'odoo-company-id',
            'desc_tip' => esc_html__( 'This is the company you wish to synchronize to.', 'woocommerce' )
        ),
        'odoo_locations_wl' => array(
            'title'    => esc_html__( 'Lacations Whitelist', 'woocommerce' ),
            'desc'     => esc_html__( 'The stock locations to synchronize (seperated by commas). Leave empty for all.', 'woocommerce' ),
            'id'       => 'odoo_locations_wl',
            'type'     => 'textarea',
            'class'    => 'odoo-locations-wl',
            'desc_tip' => esc_html__( 'This are the stock locations to synchronize (seperated by commas).', 'woocommerce' )
        ),
        'section_end' => array(
            'type' => 'sectionend',
            'id'   => 'odoo_cron_event_section'
